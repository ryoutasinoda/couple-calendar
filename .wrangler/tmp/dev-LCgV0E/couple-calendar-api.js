...
// .wrangler/tmp/bundle-nVUhlv/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-nVUhlv/middleware-insertion-facade.js
init_modules_watch_stub();
...
var VALID_TARGETS = /* @__PURE__ */ new Set(["husband", "wife", "both"]);
var VALID_ROLES = /* @__PURE__ */ new Set(["husband", "wife"]);
var VALID_ICONS = /* @__PURE__ */ new Set([
  "calendar",
  "hospital",
  "heart",
  "work",
...
var VALID_SELF_TEST_TYPES = /* @__PURE__ */ new Set(["ovulation", "pregnancy"]);
var VALID_SELF_TEST_RESULTS = /* @__PURE__ */ new Set(["negative", "positive", "pending"]);
var CYCLE_AUTO_CLOSE_DAYS = 60;
var ALLOWED_ORIGINS = /* @__PURE__ */ new Set([
  "https://couple-calendar-atu.pages.dev"
]);
var PUSH_SUBSCRIPTIONS_TABLE = `
...
var getCorsHeaders = /* @__PURE__ */ __name((request = currentRequest) => {
...
var isLocalRequest = /* @__PURE__ */ __name((request) => {
  const origin = request.headers.get("Origin") ?? "";
  const host = request.headers.get("Host") ?? "";
  return /localhost|127\.0\.0\.1/.test(origin) || /localhost|127\.0\.0\.1/.test(host);
}, "isLocalRequest");
var buildSessionCookie = /* @__PURE__ */ __name((request, sessionId, isClear = false) => {
  const value = isClear ? "" : encodeURIComponent(sessionId);
  const maxAge = isClear ? 0 : Math.floor(SESSION_TTL_MS / 1e3);
...
var setSessionCookie = /* @__PURE__ */ __name((response, request, sessionId) => {
  response.headers.append("Set-Cookie", buildSessionCookie(request, sessionId));
  return response;
}, "setSessionCookie");
var clearSessionCookie = /* @__PURE__ */ __name((response, request) => {
...
var getCurrentUser = /* @__PURE__ */ __name(async (request, db) => {
  ...
  const sessionRow = await db.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > ?").bind(sessionId, (/* @__PURE__ */ new Date()).toISOString()).first();
...
var requireAuth = /* @__PURE__ */ __name(async (request, db) => {
...
var generateInviteCode = /* @__PURE__ */ __name(() => {
...
var createDefaultCategories = /* @__PURE__ */ __name(async (db, coupleId) => {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  for (const category of DEFAULT_CATEGORIES) {
    await db.prepare("INSERT INTO categories (couple_id, name, icon, color, is_default, created_at) VALUES (?, ?, ?, ?, 1, ?)").bind(coupleId, category.name, category.icon, category.color, now).run();
  }
...
var couple_calendar_api_default = {
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/health") {
        return jsonResponse({ ok: true, name: "couple-calendar-api", time: (/* @__PURE__ */ new Date()).toISOString() });
      }
      ...
      if (path === "/api/couple/create") {
        ...
        const coupleResult = await env.DB.prepare("INSERT INTO couples (invite_code_hash, invite_code_expires_at, created_at) VALUES (?, ?, ?)").bind(inviteCodeHash, new Date(Date.now() + SESSION_TTL_MS).toISOString(), (/* @__PURE__ */ new Date()).toISOString()).run();
        await env.DB.prepare("INSERT INTO couple_members (couple_id, user_id, role, created_at) VALUES (?, ?, ?, ?)").bind(coupleResult.meta.last_row_id, auth.user.id, role, (/* @__PURE__ */ new Date()).toISOString()).run();
        await createDefaultCategories(env.DB, coupleResult.meta.last_row_id);
        return jsonResponse({
          ok: true,
      ...
      if (path === "/api/couple/join") {
        ...
        if (!matchedCouple.invite_code_expires_at || matchedCouple.invite_code_expires_at <= (/* @__PURE__ */ new Date()).toISOString()) {
        ...
        await env.DB.prepare("INSERT INTO couple_members (couple_id, user_id, role, created_at) VALUES (?, ?, ?, ?)").bind(matchedCouple.id, auth.user.id, role, (/* @__PURE__ */ new Date()).toISOString()).run();
      ...
      if (path === "/api/couple/invite-code/regenerate") {
        ...
        const inviteCode = generateInviteCode();
        const inviteCodeHash = await hashValue(inviteCode);
        await env.DB.prepare("UPDATE couples SET invite_code_hash = ?, invite_code_expires_at = ? WHERE id = ?").bind(inviteCodeHash, new Date(Date.now() + SESSION_TTL_MS).toISOString(), auth.user.coupleId).run();
      ...
      if (path === "/api/couple") {
        ...
        const userRows = await Promise.all((members.results ?? []).map(async (memberRow) => {
      ...
      if (categoryMatch) {
        ...
        if (request.method === "POST" && !categoryId) {
          ...
          const now = (/* @__PURE__ */ new Date()).toISOString();
          const result = await env.DB.prepare("INSERT INTO categories (couple_id, name, icon, color, is_default, created_at) VALUES (?, ?, ?, ?, 0, ?)").bind(auth.user.coupleId, name, icon, color, now).run();
      ...
      if (periodMatch) {
        ...
        if (request.method === "POST" && !periodId) {
          ...
          const startDate = typeof payload.start_date === "string" ? payload.start_date : "";
          if (!isValidDateString(startDate)) return jsonResponse({ error: "start_date \u306E\u5F62\u5F0F\u304C\u4E0D\u6B63\u3067\u3059\u3002" }, 400);
          const openPeriod = await env.DB.prepare("SELECT * FROM period_records WHERE couple_id = ? AND end_date IS NULL ORDER BY start_date DESC LIMIT 1").bind(auth.user.coupleId).first();
          if (openPeriod) {
            return jsonResponse({ error: "\u65E2\u306B\u9032\u884C\u4E2D\u306E\u751F\u7406\u671F\u9593\u304C\u3042\u308A\u307E\u3059\u3002" }, 409);
          }
          const now = (/* @__PURE__ */ new Date()).toISOString();
          const openCycle = await env.DB.prepare("SELECT * FROM cycles WHERE couple_id = ? AND end_date IS NULL ORDER BY start_date DESC LIMIT 1").bind(auth.user.coupleId).first();
          if (openCycle) {
            const previousDay = new Date(new Date(startDate).getTime() - 864e5).toISOString().slice(0, 10);
            await env.DB.prepare("UPDATE cycles SET end_date = ?, updated_at = ? WHERE id = ?").bind(previousDay, now, openCycle.id).run();
          }
          const periodResult = await env.DB.prepare("INSERT INTO period_records (couple_id, start_date, end_date, created_at, updated_at) VALUES (?, ?, NULL, ?, ?)").bind(auth.user.coupleId, startDate, now, now).run();
          await env.DB.prepare("INSERT INTO cycles (couple_id, period_record_id, start_date, end_date, treatment_type, result, is_manual_override, created_at, updated_at) VALUES (?, ?, ?, NULL, NULL, NULL, 0, ?, ?)").bind(auth.user.coupleId, periodResult.meta.last_row_id, startDate, now, now).run();
        ...
        if (request.method === "PUT" && periodId !== null) {
          ...
          if (endDate && !isValidDateString(endDate)) return jsonResponse({ error: "end_date \u306E\u5F62\u5F0F\u304C\u4E0D\u6B63\u3067\u3059\u3002" }, 400);
          const now = (/* @__PURE__ */ new Date()).toISOString();
          await env.DB.prepare("UPDATE period_records SET start_date = ?, end_date = ?, updated_at = ? WHERE id = ?").bind(startDate, endDate || null, now, periodId).run();
          const relatedCycle = await env.DB.prepare("SELECT * FROM cycles WHERE couple_id = ? AND period_record_id = ? ORDER BY start_date DESC LIMIT 1").bind(auth.user.coupleId, periodId).first();
          if (relatedCycle) {
            const nextEndDate = endDate || relatedCycle.end_date;
            await env.DB.prepare("UPDATE cycles SET start_date = ?, end_date = ?, updated_at = ? WHERE id = ?").bind(startDate, nextEndDate || null, now, relatedCycle.id).run();
          }
      ...
      if (cycleMatch) {
        ...
        if (request.method === "GET" && !cycleId) {
          const staleThreshold = new Date(Date.now() - CYCLE_AUTO_CLOSE_DAYS * 864e5).toISOString().slice(0, 10);
          await env.DB.prepare("UPDATE cycles SET end_date = start_date, updated_at = ? WHERE couple_id = ? AND end_date IS NULL AND start_date <= ?").bind((/* @__PURE__ */ new Date()).toISOString(), auth.user.coupleId, staleThreshold).run();
          const rows = await env.DB.prepare("SELECT * FROM cycles WHERE couple_id = ? ORDER BY start_date DESC").bind(auth.user.coupleId).all();
          const cycles = await Promise.all((rows.results ?? []).map(async (cycle) => {
        ...
        if (request.method === "PUT" && cycleId !== null) {
          ...
          const manualOverride = typeof payload.start_date === "string" || typeof payload.end_date === "string" ? 1 : existing.is_manual_override ? 1 : 0;
          if (!isValidDateString(startDate)) return jsonResponse({ error: "start_date \u306E\u5F62\u5F0F\u304C\u4E0D\u6B63\u3067\u3059\u3002" }, 400);
          if (endDate && !isValidDateString(endDate)) return jsonResponse({ error: "end_date \u306E\u5F62\u5F0F\u304C\u4E0D\u6B63\u3067\u3059\u3002" }, 400);
          await env.DB.prepare("UPDATE cycles SET start_date = ?, end_date = ?, treatment_type = ?, result = ?, is_manual_override = ?, updated_at = ? WHERE id = ?").bind(startDate, endDate || null, treatmentType || null, result || null, manualOverride, (/* @__PURE__ */ new Date()).toISOString(), cycleId).run();
...
var middleware_insertion_facade_default = couple_calendar_api_default;
...
*/
//# sourceMappingURL=couple-calendar-api.js.map
